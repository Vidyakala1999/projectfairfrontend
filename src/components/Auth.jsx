...
function Auth({ register }) {
  ...
  const handleLogin =async (e)=>{
    ...
    if( !email || !password){
      ...
      if(result.status===200){
        console.log(result);
         alert(`${result.data.username} has logined successfully`)
        ...
        <div className='card shadow p-5 ' style={{ backgroundColor: 'beige' }}>
          ...
          <div className='row align-items-center'>
            <div className='col-lg-6'>
              <img src="https://static.vecteezy.com/system/resources/previews/027/868/584/non_2x/flat-developer-team-setup-for-user-login-app-with-mobile-concept-online-login-form-login-page-user-profile-access-to-account-concept-flat-illustration-on-a-white-background-vector.jpg" alt="" className='rounded-start w-100' />
                ...
                <Form className='text-dark w-100'>
                  {
                    ...
                    <Form.Control type="text" className='text-danger' placeholder="Enter username" value={userData.username} onChange={e=>setUserData({...userData,username:e.target.value})} />
                    </Form.Group>
                  }
                    <Form.Group className="mb-3" controlId="formBasicEmail">
                    <Form.Control type="email" className='text-danger' placeholder="Enter mail" value={userData.email} onChange={e=>setUserData({...userData,email:e.target.value})}/>
                    </Form.Group>

                    <Form.Group className="mb-3" controlId="formBasicEmail">
                    <Form.Control type="password" className='text-danger'  placeholder="Enter password" value={userData.password} onChange={e=>setUserData({...userData,password:e.target.value})}/>
                    </Form.Group>
